...
we note two things. First, there is one `NA` step length, this is expected, because we are still in a point representation (i.e., there is no step length for the last relocation). Secondly, the range is quiet large from 0 to almost 5km. Before looking at step lengths in any further detail, we will have to make sure the sampling rate is more or less regular.
