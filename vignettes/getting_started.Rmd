...
We beginn with loading and inspecting the data.
...
1. check if any coordinates are missing (and if so, remove the relocations), 
...
The column names of the data set already indicate the CRS of the data. We cann add this information when creating a track.
...
### A note on pipes (`%>%`)
`amt` was heavily inspired through workflows suggested by the popular packages from the `tidyverse`. The above steps could easily be connected using pipes. Note that result will be exactly the same, the only difference is in programming style.
...
Remember, that `tracks_xy*` behaving like regular `data.frames`. This means that we can use all data manipulation verbs that we are used from `base` R or `tidyverse`. For example we an filter a track based on some characteristic. As an example we extract all relocations from Mai.
...
If we set the CRS when creating a track (we can verify this with `has_crs`), we can transform the CRS of the coordinates with the function `transform_coords` (a wrapper arround `sp::spTransform`). For illustration we will transform the CRS of `tr2` to geographical coordinates (EPSG:4326).
...
we note two things. First, there is one `NA` step length, this is expected, because we are stil in a point representation (i.e., there is no step length for the last relocation). Secondly, the range is quiet large from 0 to almost 5km. Before looking at step lengths in any further detail, we will have to make sure the sampling rate is more or less regular.

The funciton `summarize_sampling_rate` provides an easy way to look at the sampling rate.
