...
`amt` contains a simulated habitat `RasterStack` and location data simulated under (1) an HSF and (2) an iSSF to demonstrate the UHC plot workflow.

The `RasterStack` (`uhc_hab`) contains 7 layers:  
...
avail_train <- random_points(st_as_sf(st_as_sfc(st_bbox(uhc_hab))),
                             n = nrow(train) * 10)

avail_test <- random_points(st_as_sf(st_as_sfc(st_bbox(uhc_hab))),
                            n = nrow(test) * 10)
...
uhc_hsf_wrong <- prep_uhc(object = hsf_wrong, test_dat = test_dat,
                          n_samp = 20, verbose = FALSE) # In reality n_samp should be around 1000.
...
Fieberg, J.R., Forester, J.D., Street, G.M., Johnson, D.H., ArchMiller, A.A., and Matthiopoulos, J. 2018. Used-habitat calibration plots: A new procedure for validating species distribution, resource selection, and step-selection models. *Ecography* 41:737–752. [doi: 10.1111/ecog.03123](https://doi.org/10.1111/ecog.03123).
